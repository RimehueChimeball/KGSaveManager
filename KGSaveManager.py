...
class KGSaveManager:
    ...
    def auto_save_action(self):
        ...
        if self.slot_info[slot].get('exists'):
            if not messagebox.askyesno(
                    APP_NAME, self.t("msg.auto_overwrite",
    ...
    def manual_save_action(self):
        ...
        row = ttk.Frame(top)
        row.pack(fill=tk.X)
        ttk.Label(row, text=str(TEMP_FOLDER.resolve()),
        ...
        ttk.Button(btns, text=self.t("msg.btn_ok"),
                   command=self._manual_confirm).pack(side=tk.RIGHT, padx=(0, 8))

        win.protocol("WM_DELETE_WINDOW", self._manual_close)
        win.after(700, self._manual_poll)

    def _manual_copy_path(self):
        self._copy_to_clipboard(str(TEMP_FOLDER.resolve()))
        self.log(self.t("msg.path_copied", path=TEMP_FOLDER),
                 self.t("tag.save"))
    ...
    def _manual_poll(self):
        ...
        if ctx.get("detecting_path") is None:
            try:
                now = set(os.listdir(TEMP_FOLDER))
            except OSError:
                now = set()
    ...
    def _manual_confirm(self):
        ...
        if not content:
            messagebox.showwarning(APP_NAME, self.t("msg.paste_empty"),
                                   parent=ctx["win"])
            return
        if len(content) > MAX_SAVE_SIZE:
            messagebox.showerror(
                ...
                parent=ctx["win"])
            return
        ok, _ = validate_save_text(content)
        if not ok:
            if not messagebox.askyesno(
                    APP_NAME, self.t("msg.invalid_ask"), parent=ctx["win"]):
                return
    ...
    def _manual_finish(self, content, detected, invalid_key):
        ...
        if not ok and invalid_key:
            if win is None or not messagebox.askyesno(
                    APP_NAME, self.t(invalid_key), parent=win):
