...
- **主版本 · HTML 界面**（`KGSaveManager.py` + `webapp/`）——界面是本地网页，
  以浏览器应用窗口打开（也可在配置页改成浏览器标签页）；窗口标题与图标由页面决定
  （`<title>`、`favicon.ico`、`manifest.webmanifest`），也可以在浏览器菜单里把它
  装成独立应用。
...
- 配置：语言、浏览器、游戏目录、端口、游戏窗口位置（独立应用窗口 / 浏览器标签页）、
  自动续玩（关掉游戏时存进度，下次打开自动恢复；只作用于「启动游戏」打开的窗口）；
  修改自动保存
