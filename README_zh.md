...
- 存档管理：10 个存档位，自动按文件名 `名字_槽位号.kgsav` 识别；自动存档（经本地桥直取游戏存档）、手动存档（放文件/粘贴文本）、读档、改名、刷新、备注自动保存、检查异常文件
- 启动游戏：本地静态 Web 服务（仅 127.0.0.1）+ WebSocket 存档桥，一键启动并在浏览器新窗口打开，含实时服务日志
...
- 自动存档：用 KGSM 启动游戏后点「自动存档」，游戏当前存档经桥直接写入选中槽位（覆盖时确认）
- 手动存档：弹窗上部显示临时文件夹（可「复制路径」），把游戏导出的存档文件放进去即自动保存并关闭窗口、弹出「检测到存档」；下部可粘贴存档文本后点「确定」；关闭窗口表示取消
