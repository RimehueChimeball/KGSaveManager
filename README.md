...
- 启动游戏页：本地 Web 服务（仅绑定 127.0.0.1），一键启动并打开浏览器，实时服务日志
- 存档管理页：6 个存档位，存/读档、备注自动保存、检查异常文件
- 配置页：语言（中/英翻译，首次按系统语言探测）、游戏目录、固定端口、首页指定存档，**修改自动保存**到 `kgsm_data/kgsm_config.json`
...
- 底部：「第一次使用？<配置程序>」跳到配置页；「下载游戏」打开 Kittens Game 的 GitHub 仓库。
...
语言切换立即生效并重建界面；游戏目录、固定端口、首页指定存档修改后自动写入配置文件。
...
- 游戏下载链接为 `KGSaveManager.py` 顶部常量 `GAME_DOWNLOAD_URL`，可自行更换。
