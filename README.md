...
- 多标签页界面：默认「存档管理」，另有「启动」页——一键创建本地静态 Web 服务并在浏览器新窗口打开。
...
1. 切换到「启动」标签页。
2. 设置服务目录（默认 `web/`，不存在会自动创建）与端口（留空自动选择空闲端口）。
3. 点击「启动并打开浏览器」，程序会创建本地服务（仅绑定 `127.0.0.1`）并在浏览器新窗口打开。
4. 需要浏览的站点 / 游戏文件放入服务目录即可，例如把入口文件命名为 `index.html`。
...
├─ README.md
├─ web/                   # 「启动」页默认服务目录（内置示例 index.html）
