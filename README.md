...
- 存档管理页：10 个存档位（前缀随语言显示 `存档01.`/`Save01.`，可自定义名字并同步改名存档文件；无文件不显示默认名），存/读档/改名、备注自动保存、检查异常文件，列表支持滚轮
...
- 剪贴板：内置 Tkinter 实现，零第三方依赖
- 数据集中：存档/配置全部在 `kgsm_data/`，备份或迁移只移动它

## 环境要求

- Windows
- Python 3.8+（含 `tkinter`，随 Python 标准库自带，**无第三方依赖**）
...
- 底部：「第一次使用？[配置程序]」按钮跳到配置页；两个「下载游戏」GitHub 链接（作者原版 nuclear-unicorn / 社区版 kitten-science，均标注 GitHub）。
...
3. 「改名」：重命名当前存档位（显示名与磁盘文件 `名字_N.kgsav` 同步改名）；
4. 备注在输入框填写即自动保存；「检查异常文件」查看不合规文件。
   存档位列表支持鼠标滚轮滚动。
