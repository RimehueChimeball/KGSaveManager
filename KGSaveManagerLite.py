...
适合只想要一个普通桌面窗口、或系统里没有 Edge/Chrome 的情况。

与主版本（`KGSaveManager.py` + `webapp/`，HTML 界面）共用 `core/` 的全部逻辑、
同一份 `kgsm_data/` 与同一套配置，功能一致，只是界面不同。
...
web_server.py（本地 Web 服务）、web_bridge.py（页面桥）、savecodec.py（存档编解码）、
downloader.py（下载）、kgsm_logging.py（运行日志）、utils.py（DPI）。
"""
